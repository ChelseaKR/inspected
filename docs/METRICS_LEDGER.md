...
| Next deliberate refresh | not begun | the artifact the row above left in place: four sources pinned 2026-08-23, nine fetched DINS fields, three-repair census, coordinate-county agreement measured | not measured, because the stream has not run | The refresh procedure in `docs/RUNBOOK.md` is the metric: artifact diff against the replaced pin, dated `PROVENANCE.md` entry. An earlier instance of this row was superseded by the row above when trigger 3 fired, and lost three of its five cells in the edit |
...
| Published values in the current pin | 6,582 leaves in `published/measurements.json`, comparable by `wildfire_service_territory_overlap.artifact_diff`, of which 1,205 carry a string this repository wrote |
