...
# Bare `make` runs the one gate, and now says so. It already did: `verify` is the first
# target in the file, so it has been the default goal since the file was written. Making
# `help` the default instead would change what a habitual `make` does from running every
# gate to printing a list, and a command that quietly starts doing something else is the
# failure mode this repository is built against. The goal is declared rather than
# inherited from line order, so deleting a target above cannot move it by accident.
.DEFAULT_GOAL := verify

# CI and `make verify` run the same list. The two MUST stay identical.
# See CONTRIBUTING.md and .github/workflows/ci.yml.
verify: lock-check sync lint format typecheck test audit report-offline determinism ## the one gate: lock-check, sync, lint, format, typecheck, test, audit, report-offline, determinism, which is the list CI runs

# Self-documenting: a target is listed here because its own line carries a `##`
# description, so adding a target and documenting it are one edit rather than two.
# tests/test_provenance_and_standards.py runs this target and reads what it printed, so a
# target added without a description fails the gate rather than going quietly undocumented.
